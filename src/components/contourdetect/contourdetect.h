...
struct ContourDetectGuiInt;
class ContourDetectPrivate;
typedef Context<ContourDetectGuiInt> ContourDetectCtx;
/**
*   @brief The class provides abstraction layer for CAN BUS hardware
*/
class ContourDetect : public QObject, public ComponentInterface {
    Q_OBJECT
    Q_DECLARE_PRIVATE(ContourDetect)

public:
    explicit ContourDetect();
    ~ContourDetect();
...
signals:
    ...
    void frameSent(bool status, const QImage& frame);
...
private:
    QScopedPointer<ContourDetectPrivate> d_ptr;
};