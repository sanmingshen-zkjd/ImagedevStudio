...
#include "gui/contourdetectguiimpl.h"
//#include <propertyfields.h>

class ContourDetect;

class ContourDetectPrivate : public QObject {
    Q_OBJECT
    Q_DECLARE_PUBLIC(ContourDetect)

public:
    ContourDetectPrivate(
        ContourDetect* q, ContourDetectCtx&& ctx = ContourDetectCtx(new ContourDetectGuiImpl))
        : _ctx(std::move(ctx))
        ,_ui(_ctx.get<ContourDetectGuiInt>()) 
        ,q_ptr(q)
    {
       // initProps();
...
public:
  ...
  //  QVector<QImage> _sendQueue;
    ...
    ContourDetectCtx _ctx;
    ContourDetectGuiInt& _ui;
